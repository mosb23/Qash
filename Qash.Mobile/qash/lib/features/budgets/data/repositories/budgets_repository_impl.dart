...
class BudgetsRepositoryImpl implements BudgetsRepository {
  ...
  @override
  Future<Result<BudgetStatusEntity>> getBudgetById(String budgetId) async {
    final response = await _remoteDataSource.getBudgetById(budgetId);
  ...
  @override
  Future<Result<String>> updateBudget(BudgetUpdateData data) async {
    final resolvedUserId = data.userId.isNotEmpty
        ? data.userId
        : await _storage.getUserId() ?? '';
    ...
    final response = await _remoteDataSource.updateBudget(
      data.budgetId,
      BudgetUpdateRequestModel.fromDomain(
        BudgetUpdateData(
